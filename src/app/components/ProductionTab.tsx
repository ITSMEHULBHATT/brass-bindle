...
import { Search, ChevronDown, ChevronRight, X, PackageOpen } from "lucide-react";
    ...
    <div className="space-y-3 px-4 py-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-[10px] border border-border bg-card px-9 text-[14px] text-foreground placeholder:text-muted-foreground outline-none focus:border-primary focus:shadow-[0_0_0_3px_rgba(26,43,74,0.08)]"
      ...
      <div className="flex items-baseline justify-between px-1 text-[12px]">
        ...
        <span className="text-sm font-bold tabular-nums text-primary">{grandTotal} units total</span>
      </div>

      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
          <PackageOpen className="size-12" strokeWidth={1.5} style={{ color: "#D1D5DB" }} />
          <p className="text-[15px] font-semibold" style={{ color: "#374151" }}>
            {rows.length === 0 ? "Nothing to produce" : "No matches"}
          </p>
          <p className="text-[13px] text-muted-foreground">
            {rows.length === 0
              ? "All open orders are fully fulfilled."
                        ...
                        {row.breakdown.map((b, i) => {
                          ...
                          return (
                            <tr key={i} className="border-t border-[#F3F4F6] first:border-t-0">
                              <td className="py-1.5 pr-2 font-medium" style={{ color: "#374151" }}>
                                {b.customerName}
                              </td>
                              ...
                              <td className="py-1.5 text-right font-bold tabular-nums text-primary">
                                {b.remaining}
                              </td>
                            </tr>
                          );
