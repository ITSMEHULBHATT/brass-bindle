...
export function ArchiveTab({
  ...
  if (sorted.length === 0) {
    return (
      <EmptyState
        icon={<ArchiveIcon className="size-12" strokeWidth={1.5} />}
        title="No completed orders"
        subtitle="Fulfilled orders appear here automatically."
      />
    );
  }

  return (
    <div className="space-y-[10px] px-4 py-3">
      {sorted.map((o) => {
        ...
        return (
          ...
          >
            ...
            <ul className="border-t border-[#F3F4F6] px-3 py-2 text-xs text-muted-foreground">
              {o.items.map((it, i) => (
...
function EmptyState({
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
      ...
      <h3 className="mt-3 text-[15px] font-semibold text-[#374151]">{title}</h3>
